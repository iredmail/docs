...
iRedMail Eazy (the deployment and support platform) maintains core config files,
...
* If software does not support loading settings from multiple config files,
  ...
  `/opt/iredmail/custom/postfix/custom.sh`, it's ran by iRedMail Eazy
...
- `/opt/iredmail/custom/nginx/custom.sh`:
    - a bash shell script for advanced customization. This file will be executed
      every time iRedMail Eazy deploys / upgrades the Nginx component.
