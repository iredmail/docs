...
* Download the latest package with `wget` command, extract download package and
  run a script to upgrade it. Note: We use version `3.0.7` for example here, `3.0.7.tar.gz`.

```
cd /root/
wget https://github.com/iredmail/mlmmjadmin/archive/3.0.7.tar.gz
tar zxf 3.0.7.tar.gz
cd mlmmjadmin-3.0.7/tools/
