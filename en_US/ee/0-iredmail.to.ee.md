...
* `/etc/postfix/main.cf` and `/etc/postfix/master.cf`
    ...
    - Write your new custom settings for `/etc/postfix/master.cf`
      ...
      perform upgrade, re-perform full deployment, or re-deploy Postfix.
    - [__NOT RECOMMENDED__] If you need to modify settings generated by EE, you can maintain your own
      copy of `main.cf` and `master.cf` under `/opt/iredmail/custom/postfix/` directory,
      but this is __NOT RECOMMENDED__, because EE may introduce new parameters
      in `main.cf` and `master.cf`, your own copies won't have the new parameters
      automatically and it may cause Postfix not working as expected.
