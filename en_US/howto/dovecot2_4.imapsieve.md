...
!!! warning
    ...
    - This tutorial is NOT applicable for Dovecot 2.3 due to incompatible config syntax.
...
- Dovecot version 2.4.0 or later, 2.4.x.

    `imap_sieve` plugin is available in Dovecot 2.4.0 and later releases.
...
* Add required settings for `imap_sieve` in `sieve_plugins {}` section.
* Add required settings for `pipe an env` in `sieve_global_extensions {}` section.
* Add required settings for `pipe dir in` `dovecot.conf`.
