# Installeer iRedMail op FreeBSD
...
> Als je iRedMail wilt installeren binnen FreeBSD __Jail__, gebruik dan
> deze installatiegids:
> [Installeer iRedMail binnen FreeBSD Jail (met ezjail)](./install.iredmail.on.freebsd.with.jail.html)
...
* Een __NIEUWE__, minimale, werkende installatie van FreeBSD, 
  geen extra ports of geïnstalleerde binaire
  packages. Ondersteunde versies staan op de
...
Opmerking: Alle packages worden geïnstalleerd van de ports tree.
...
Op FreeBSD wordt de hostname bepaald door 2 bestanden: `/etc/rc.conf`, `/etc/hosts`.

* `/etc/rc.conf`: bepaalt hostname.

```
# Deel van het bestand: /etc/rc.conf
hostname="mx.example.com"
```

* `/etc/hosts`: hostname <=> IP-adres mapping.
...
### Update ports tree met portsnap

```
# portsnap fetch extract update
```

### Installeer de package `bash-static`, iRedMail heeft het nodig om correct te werken
...
* Kies backend dat gebruikt wordt om e-mail accounts op te slaan. Je kunt alle accounts beheren met iRedAdmin, ons webgebaseerde iRedMail administratiepaneel
...
!!! boodschap "aan alle MySQL/MariaDB/PostgreSQL gebruikers"
    ...
    zal het installatieprogramma een willekeurig, sterk password voor je bedenken. Je kunt het terugvinden in het bestand `iRedMail.tips`.
